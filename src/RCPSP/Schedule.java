	...
	public void initializeJobListe(Job[] jobs) {
		...
		while (count != jobs.length) {
			...
			for (int i = 0; i < nachfolgerAkt.size(); i++) { // Liste der nachfolger wird durchlaufen
				Job aktuellerNachfolgerJob = Job.getJob(jobs, nachfolgerAkt.get(i)); // nachfolger neuer Job (nicht in
																						// JObliste)
				ArrayList<Integer> vorgaengerAkt = aktuellerNachfolgerJob.vorgaenger; // vorgänger dieses nachfolgers
																						// wird in Liste gespreichert
				...
				for (int j = 0; j < vorgaengerAkt.size(); j++) { // Vorgänger wird durchlaufen
					...
					if (!found) { // wenn nicht alle Vorgänger gefunden wurden springt er in die for und sucht den
									// nächsten Nachfolger (passiert bis ein passender Nachfolger gefunden wurde)
						alleVorgaenger = false;
						break;
					}

				}
				if (alleVorgaenger) { // wenn alle Vorgänger gefunden wurden wird der nächste nachfolgerJob zu
										// Planbaren Jobs hinzugefügt
					eligibleJobs.add(Job.getJob(jobs, nachfolgerAkt.get(i)));
				}
			}
		}
